...
cmake_definition+= -DOUTPUT=$(OUTPUT_DIR)
...
build:Makefile
	cd bl_mcu_sdk && cmake -S . -B build -G "Unix Makefiles" $(cmake_definition)
	cd bl_mcu_sdk/build && make -j4
...
download:
	./bl_mcu_sdk/tools/bflb_flash_tool/bflb_mcu_tool $(FLASH_DOWNLOAD_CONFIG)
